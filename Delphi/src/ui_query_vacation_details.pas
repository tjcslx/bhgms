unit ui_query_vacation_details;
...
type
  TForm_Query_Vacation_Details = class(TForm)
...
var
  Form_Query_Vacation_Details: TForm_Query_Vacation_Details;

implementation

{$R *.dfm}

procedure TForm_Query_Vacation_Details.Button_DeselectAllClick(Sender: TObject);
begin
  if JvCheckedComboBox_BtofId.Items.Count > 0 then
    JvCheckedComboBox_BtofId.SetUnCheckedAll();
end;

procedure TForm_Query_Vacation_Details.Button_InvertAllClick(Sender: TObject);
begin
  if JvCheckedComboBox_BtofId.Items.Count > 0 then
    JvCheckedComboBox_BtofId.SetInvertAll();
end;

procedure TForm_Query_Vacation_Details.Button_QueryClick(Sender: TObject);
var
  d: TFormatSettings;
  ...
  if DateTimePicker_BeginDate.Date > DateTimePicker_EndDate.Date then
    ...
    if ((JvCheckedComboBox_VacationType.Text = '') or (JvCheckedComboBox_ApproveStatus.Text = '') or (JvCheckedComboBox_BtofId.Text = '')) then
      ...
      begin
        ...
        DataModule_sjhf.ADOQuery_SjhfDb.SQL.Text := 'SELECT B1.BTO_NAME,(CASE WHEN F1.FA_ISSUER_INFACT IS NOT NULL THEN B2.BTOF_NAME||''（''||B3.BTOF_NAME||''）'' ELSE B2.BTOF_NAME END) AS BTOF_ISSUER,D2.DVT_NAME,D1.DAS_NAME,SUM(F2.FAVS_VACATION_HOUR)*60+SUM(F2.FAVS_VACATION_MIN) AS MINS FROM ';
        DataModule_sjhf.ADOQuery_SjhfDb.SQL.Add('BHGMS_F_APPROVE_VACATION_SUB F2,BHGMS_F_APPROVE F1,BHGMS_U_OFFICER B3,BHGMS_U_OFFICER B2,BHGMS_U_ORGAN B1,BHGMS_D_VACATION_TYPE D2,BHGMS_D_APPROVE_STATUS D1 WHERE F1.FA_STATUS=D1.DAS_ID AND F2.FAVS_TYPE=D2.DVT_ID AND F1.FA_ISSUER=B2.BTOF_ID AND ');
        DataModule_sjhf.ADOQuery_SjhfDb.SQL.Add('F1.FA_ISSUER_INFACT=B3.BTOF_ID(+) AND B1.BTO_ID=B2.BTO_ID AND F1.FA_ID=F2.FA_ID AND (B2.BTOF_NAME ' + GetSelectedSQLString(JvCheckedComboBox_BtofId, JvCheckedComboBox_BtofId.CheckedCount) + ' OR B3.BTOF_NAME ' + GetSelectedSQLString(JvCheckedComboBox_BtofId, JvCheckedComboBox_BtofId.CheckedCount) + ') AND D1.DAS_NAME ' + GetSelectedSQLString(JvCheckedComboBox_ApproveStatus, JvCheckedComboBox_ApproveStatus.CheckedCount) + ' AND D2.DVT_NAME ' + GetSelectedSQLString(JvCheckedComboBox_VacationType, JvCheckedComboBox_VacationType.CheckedCount));
        DataModule_sjhf.ADOQuery_SjhfDb.SQL.Add(' GROUP BY B1.BTO_NAME,(CASE WHEN F1.FA_ISSUER_INFACT IS NOT NULL THEN B2.BTOF_NAME||''（''||B3.BTOF_NAME||''）'' ELSE B2.BTOF_NAME END) ORDER BY F2.FAVS_ISSUEDATE,F2.FAVS_ENDDATE,F2.FAVS_BEGINTIME,F2.FAVS_ENDTIME');
        ...
        Cell_Query_Vacation.ClearArea(1, 6, 9, 6, 0, 1);
          ...
          begin
            ...
            for current_row := 2 to rows do
              begin
                ...
                Cell_Query_Vacation.S(2, current_row, 0, DataModule_sjhf.ADOQuery_SjhfDb.FieldByName('BTOF_ISSUER').AsString);
                Cell_Query_Vacation.S(3, current_row, 0, DataModule_sjhf.ADOQuery_SjhfDb.FieldByName('FAVS_TIME_TYPE').AsString);
                Cell_Query_Vacation.SetCellDateTime(4, current_row, 0, DataModule_sjhf.ADOQuery_SjhfDb.FieldByName('FAVS_ISSUEDATE').AsString);
                Cell_Query_Vacation.SetCellDateTime(5, current_row, 0, DataModule_sjhf.ADOQuery_SjhfDb.FieldByName('FAVS_ENDDATE').AsString);

                if DataModule_sjhf.ADOQuery_SjhfDb.FieldByName('FAVS_BEGINTIME').AsString <> '' then
                  Cell_Query_Vacation.S(6, current_row, 0, LeftStr(DataModule_sjhf.ADOQuery_SjhfDb.FieldByName('FAVS_BEGINTIME').AsString, 2) + ':' + RightStr(DataModule_sjhf.ADOQuery_SjhfDb.FieldByName('FAVS_BEGINTIME').AsString, 2));

                if DataModule_sjhf.ADOQuery_SjhfDb.FieldByName('FAVS_ENDTIME').AsString <> '' then
                  Cell_Query_Vacation.S(7, current_row, 0, LeftStr(DataModule_sjhf.ADOQuery_SjhfDb.FieldByName('FAVS_ENDTIME').AsString, 2) + ':' + RightStr(DataModule_sjhf.ADOQuery_SjhfDb.FieldByName('FAVS_ENDTIME').AsString, 2));

                Cell_Query_Vacation.S(8, current_row, 0, DataModule_sjhf.ADOQuery_SjhfDb.FieldByName('DVT_NAME').AsString);
                Cell_Query_Vacation.S(9, current_row, 0, DataModule_sjhf.ADOQuery_SjhfDb.FieldByName('DAS_NAME').AsString);
...
procedure TForm_Query_Vacation_Details.Button_SelectAllClick(Sender: TObject);
begin
  if JvCheckedComboBox_BtofId.Items.Count > 0 then
    JvCheckedComboBox_BtofId.SetCheckedAll();
end;

procedure TForm_Query_Vacation_Details.FormActivate(Sender: TObject);
var
  d: TFormatSettings;
  i: Integer;
  Cell_Name: string;
begin
  ...
  if Cell_Query_Vacation.GetFileName = '' then
    Cell_Name := ExtractFilePath(Application.ExeName) + '\..\cll\cell_query_vacation_details.cll';
...
function TForm_Query_Vacation_Details.GetSelectedSQLString(jvccb: TJvCheckedComboBox;
  sc: integer): string;
var
  desc: string;
begin
...
procedure TForm_Query_Vacation_Details.JvCheckedComboBox_BtoIdChange(Sender: TObject);
var
  i: Integer;
begin
